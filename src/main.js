...
import createOl from './createTweetOl.js'
import createFrenchButton from './createFilterButton.js'
...
document.addEventListener(
  ...
  Promise.all([
    ...
    .then(function (allTweets) {
      ...
      const olFrench = createOl(tweets.filter(isFrenchTweet))
      ...
      buttonTrack.addEventListener('click', function () {
        tracking = !tracking
        if (tracking) {
          document.addEventListener('mousemove', listenerTrack)
        } else {
          document.removeEventListener('mousemove', listenerTrack)
        }
      })

      document.body.prepend(buttonFrench, buttonTrack)
